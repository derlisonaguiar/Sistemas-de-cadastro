...
function runPrisma(...args) {
  ...
  if (result.status !== 0) throw new Error(`Falha ao executar prisma ${args.join(" ")} (código ${result.status ?? "desconhecido"}).`);
}
...
try {
  ...
  for (const { tablename } of tables.rows) {
    const result = await client.query(`SELECT EXISTS (SELECT 1 FROM ${quoteIdentifier(tablename)} LIMIT 1) AS has_rows`);
    if (result.rows[0].has_rows) throw new Error(`ABORTADO: a tabela ${tablename} já possui dados. Nenhuma alteração foi aplicada.`);
  }
} finally {
  await client.end();
}
...
if (deploymentMode !== "single") console.log("Aviso: este fluxo prepara a base para /setup, que é usado somente no modo single.");
