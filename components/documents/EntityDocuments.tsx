  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (active) setError("Não foi possível carregar os documentos.");
      } finally {
  ...
  return (
    <section className="mt-6 rounded-lg border border-gray-200 bg-white p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <AdminOnly>
          <Link
            href={`/admin/documentos/importar?${new URLSearchParams({ [entityKey]: entityId })}`}
      ...
      {loading ? (
        ...
        <ul className="divide-y divide-gray-200">
          {documents.map((document) => (
            <li key={document.id} className="flex flex-wrap justify-between gap-3 py-3 text-sm">
              <Link href={`/admin/documentos/${document.id}`} className="text-[var(--admin-ink)] underline">
                {document.title}
              </Link>
              <span>
                {document.origin === "IMPORTED" ? "Importado" : "Gerado"} · {documentStatusLabels[document.status]} ·{" "}
