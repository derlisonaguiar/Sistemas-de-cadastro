...
export default function MemberExportDialog({ onClose }: { onClose: () => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const filename =
        response.headers.get("Content-Disposition")?.match(/filename="?([^";]+)"?/)?.[1] || `membros.${format}`;
  ...
  return (
    ...
    >
      <form onSubmit={submit} className="space-y-5">
        <div>
          <h2 id="member-export-title" className="text-lg font-semibold">
            Exportar membros
          </h2>
          <p className="mt-1 text-sm text-gray-600">
            Escolha os membros e as informações que serão incluídas no arquivo.
          </p>
        </div>
        ...
        <label className="block text-sm font-medium text-gray-700">
          ...
          <select
            value={status}
            disabled={exporting}
            onChange={(event) => setStatus(event.target.value as MemberExportStatus | "")}
        ...
        <fieldset>
          ...
          <div className="mt-2 flex gap-4">
            {(["csv", "xlsx", "pdf"] as const).map((value) => (
        ...
        <div className="flex justify-end gap-3">
          <button type="button" disabled={exporting} onClick={onClose} className="rounded-md border px-3 py-2 text-sm">
            Cancelar
          </button>
          <button
            type="submit"
