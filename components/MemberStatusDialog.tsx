...
export default function MemberStatusDialog<
  T extends { id: string; fullName: string; status: keyof typeof memberStatusLabels },
>({ member, onClose, onUpdated }: { member: T; onClose: () => void; onUpdated: (member: T) => void }) {
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <p className="text-sm text-gray-600">
          {member.fullName} — atual: {memberStatusLabels[member.status]}
        </p>
        <div>
          <label htmlFor="quick-status" className="block text-sm font-medium">
            Novo status
          </label>
          <select
            autoFocus
        ...
        {status === "POS_JR" && (
          <p className="text-sm text-gray-600">
            O membro deixará de ocupar cargo e diretoria. Histórico e documentos serão preservados.
          </p>
        )}
        <div>
          <label htmlFor="admin-current-password" className="block text-sm font-medium">
            Sua senha atual de ADMIN
          </label>
          <input
        ...
        <div className="flex justify-end gap-3">
          <button type="button" disabled={saving} onClick={onClose} className="rounded-md border px-3 py-2 text-sm">
            Cancelar
          </button>
          <button
            type="submit"
