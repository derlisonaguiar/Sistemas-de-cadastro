...
const statusByLabel = new Map(
  Object.entries(memberStatusLabels).flatMap(([value, label]) => [
    [key(value), value],
    [key(label), value],
  ])
);
...
type ImportMemberData = Omit<ReturnType<typeof memberSchema.parse>, "cpf"> & {
  cpf: string | null;
...
async function parseFile(file: File, organizationId: string) {
  ...
  if (!file.name || !["csv", "xlsx"].includes(extension || "") || file.size === 0 || file.size > MAX_BYTES)
    throw new Error("Envie um arquivo CSV ou XLSX de até 5 MB.");
  const fileData = await file.arrayBuffer();
  const workbook =
    extension === "csv"
      ? XLSX.read(new TextDecoder().decode(fileData), {
          type: "string",
          raw: false,
          FS: new TextDecoder().decode(fileData).split(/\r?\n/, 1)[0]?.includes(";") ? ";" : ",",
        })
  ...
  if (headers.some((header) => !headerRow.includes(key(header))))
    throw new Error("Use o modelo de importação com todos os cabeçalhos obrigatórios.");
  const records = values.slice(1).filter((row) => row.some((value) => String(value).trim() !== ""));
  if (records.length === 0 || records.length > MAX_ROWS)
    throw new Error(`A planilha deve conter entre 1 e ${MAX_ROWS} linhas.`);
  const [directorates, positions, existing] = await Promise.all([
    ...
    prisma.position.findMany({
      ...
      select: { id: true, name: true, directorateId: true, role: true },
    }),
  ...
  const seenCpf = new Set(existing.map((item) => item.cpf).filter(Boolean));
  const seenRegistration = new Set(existing.map((item) => key(item.registration)).filter(Boolean));
  const seenEmail = new Set(existing.map((item) => key(item.email)).filter(Boolean));
  return records.map((row, index): ParsedRow => {
    const value = (header: string) => String(row[headerRow.indexOf(key(header))] ?? "").trim();
    const errors: string[] = [];
    const warnings: string[] = [];
    const directorateName = value("Diretoria");
    const positionName = value("Cargo");
    ...
    if (status !== "POS_JR" && position?.directorateId && position.directorateId !== directorate?.id)
      errors.push("Cargo incompatível com a diretoria");
    const cpf = normalizeTaxId(value("CPF"));
    const registration = value("Matrícula");
    const email = value("E-mail");
    if (cpf && seenCpf.has(cpf)) errors.push("Já existe um membro cadastrado com este CPF.");
    else if (cpf) seenCpf.add(cpf);
    if (registration && seenRegistration.has(key(registration))) errors.push("Matrícula duplicada");
    else if (registration) seenRegistration.add(key(registration));
    if (email && seenEmail.has(key(email))) errors.push("E-mail já cadastrado ou duplicado");
    else if (email) seenEmail.add(key(email));
    const cpfNeedsReview = !isValidCpf(cpf);
    if (cpfNeedsReview)
      warnings.push(
        cpf
          ? "CPF inválido — o membro poderá ser importado, mas ficará pendente de correção."
          : "CPF ausente — o membro poderá ser importado, mas ficará pendente de correção."
      );
    const result = memberSchema.safeParse({
      fullName: value("Nome"),
      cpf: cpfNeedsReview ? "52998224725" : cpf,
    ...
    return {
      ...
      fullName: value("Nome"),
      data: result.success
        ? { ...result.data, cpf: cpfNeedsReview ? cpf || null : result.data.cpf, cpfNeedsReview }
        : undefined,
      errors,
      warnings,
    };
...
async function createRow(organizationId: string, row: ParsedRow) {
  ...
  await prisma.$transaction(async (tx) => {
    const position = data.positionId
      ? await tx.position.findFirst({ where: { id: data.positionId, organizationId } })
      : null;
    if (data.positionId && !position) throw new Error("Cargo não encontrado.");
    if (
      data.status === "ACTIVE" &&
      position?.role &&
      ["PRESIDENT", "VICE_PRESIDENT", "DIRECTOR"].includes(position.role)
    ) {
      const where =
        position.role === "DIRECTOR"
          ? {
              ...
              position: { role: "DIRECTOR" as const },
            }
          : { organizationId, status: "ACTIVE" as const, position: { role: position.role } };
    ...
    if (data.cpf && (await tx.member.findFirst({ where: { organizationId, cpf: data.cpf }, select: { id: true } })))
      throw new Error("Já existe um membro cadastrado com este CPF.");
    await tx.member.create({
      data: {
        ...
        email: data.email,
        cpf: data.cpf,
        cpfNeedsReview: data.cpfNeedsReview,
        phone: data.phone,
        course: data.course,
...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || !["preview", "confirm"].includes(String(action)))
      return NextResponse.json({ ok: false, message: "Arquivo ou ação inválidos." }, { status: 400 });
    ...
    for (const row of validRows) {
      try {
        await createRow(organization.id, row);
        imported++;
      } catch (error) {
        failures.push({
          line: row.line,
          error: isCpfConflict(error)
    ...
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "Erro ao importar membros." },
      { status: 400 }
    );
  }
}
