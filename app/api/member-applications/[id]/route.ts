...
export async function GET(_request: Request, context: Context) {
  ...
  const application = await prisma.memberApplication.findFirst({
    where: { id: result.id, organizationId: result.auth.profile.organizationId },
  });
...
export async function PUT(request: Request, context: Context) {
  try {
    ...
    const current = await prisma.memberApplication.findFirst({
      where: { id: result.id, organizationId: result.auth.profile.organizationId },
    });
    ...
    if (
      ...
      !(await prisma.directorate.findFirst({
        where: { id: data.directorateId, organizationId: current.organizationId },
      }))
    )
      return NextResponse.json({ ok: false, message: "Diretoria inválida." }, { status: 400 });
    const position = data.positionId
      ? await prisma.position.findFirst({ where: { id: data.positionId, organizationId: current.organizationId } })
      : null;
    if (data.positionId && !position)
      return NextResponse.json({ ok: false, message: "Cargo inválido." }, { status: 400 });
    if (position?.directorateId && position.directorateId !== data.directorateId)
...
export async function PATCH(request: Request, context: Context) {
  try {
    ...
    if (action === "REJECT") {
      const updated = await prisma.memberApplication.update({
        ...
        data: {
          ...
          reviewedById: result.auth.user.id,
          reviewedAt: new Date(),
        },
      });
      return NextResponse.json({
        ok: true,
        application: updated,
        message: "Inscrição rejeitada e mantida no histórico.",
      });
    }
    const selectedPosition = application.positionId
      ? await prisma.position.findFirst({
          where: { id: application.positionId, organizationId: application.organizationId },
        })
      : null;
    if (application.positionId && !selectedPosition)
      return NextResponse.json({ ok: false, message: "Cargo inválido." }, { status: 400 });
    if (
      application.directorateId &&
      !(await prisma.directorate.findFirst({
        where: { id: application.directorateId, organizationId: application.organizationId },
      }))
    )
      return NextResponse.json({ ok: false, message: "Diretoria inválida." }, { status: 400 });
    if (selectedPosition?.directorateId && selectedPosition.directorateId !== application.directorateId)
      return NextResponse.json(
        { ok: false, message: "O cargo não pertence à diretoria selecionada." },
        { status: 400 }
      );
    if (selectedPosition?.role === "DIRECTOR" && !application.directorateId)
      return NextResponse.json(
        { ok: false, message: "Um Diretor precisa estar vinculado a uma diretoria." },
        { status: 400 }
      );
    if (selectedPosition && ["PRESIDENT", "VICE_PRESIDENT", "DIRECTOR"].includes(selectedPosition.role)) {
      const occupied = await prisma.member.findFirst({
        where: {
          organizationId: application.organizationId,
          status: "ACTIVE",
          ...(selectedPosition.role === "DIRECTOR" ? { directorateId: application.directorateId } : {}),
          position: { role: selectedPosition.role },
        },
        select: { fullName: true },
      });
      if (occupied)
        return NextResponse.json(
          { ok: false, message: `O cargo selecionado já está ocupado por ${occupied.fullName}.` },
          { status: 409 }
        );
    }
    ...
    if (duplicate)
      return NextResponse.json(
        { ok: false, message: "Já existe um membro com o CPF, e-mail ou usuário desta inscrição." },
        { status: 409 }
      );
    const profile = await prisma.userProfile.findUnique({ where: { id: application.userId } });
    if (profile)
      return NextResponse.json(
        { ok: false, message: "Este usuário já está vinculado a uma organização." },
        { status: 409 }
      );
    const approved = await prisma.$transaction(async (tx) => {
      const member = await tx.member.create({
        data: {
          ...
          status: "ACTIVE",
          directorateId: application.directorateId,
    ...
    return NextResponse.json({
      ...
      message: "Inscrição aprovada. O membro está ativo.",
    });
  } catch (error) {
    const code = typeof error === "object" && error && "code" in error ? String(error.code) : "";
    if (code === "P2002")
      return NextResponse.json({ ok: false, message: "A aprovação criaria um registro duplicado." }, { status: 409 });
    return databaseErrorResponse(error);
  }
}
