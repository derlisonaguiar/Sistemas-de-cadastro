...
export async function POST(request: Request) {
  ...
  if (!application) return NextResponse.json({ ok: false, message: "Envie a inscrição antes da foto." }, { status: 404 });
  ...
  try { image = validateImageUpload(buffer, file); } catch { return NextResponse.json({ ok: false, message: "Envie uma imagem JPG, PNG ou WebP válida." }, { status: 400 }); }
