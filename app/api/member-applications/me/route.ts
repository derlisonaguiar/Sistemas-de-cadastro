...
export async function POST(request: Request) {
  try {
    ...
    if (profile)
      return NextResponse.json({ ok: false, message: "Esta conta já possui acesso à organização." }, { status: 409 });
    ...
    if (member)
      return NextResponse.json({ ok: false, message: "Este CPF já está cadastrado como membro." }, { status: 409 });
    const application = await prisma.memberApplication.create({
      data: { ...data, organizationId: organization.id, userId: auth.user!.id },
    });
    return NextResponse.json(
      { ok: true, application, message: "Inscrição enviada. Aguarde a análise da administração." },
      { status: 201 }
    );
  } catch (error) {
    ...
    if (code === "P2002")
      return NextResponse.json(
        { ok: false, message: "Já existe uma inscrição ou membro com estes dados." },
        { status: 409 }
      );
...
export async function PUT(request: Request) {
  try {
    ...
    if (current.status !== "PENDING")
      return NextResponse.json(
        { ok: false, message: "Somente inscrições pendentes podem ser alteradas." },
        { status: 409 }
      );
    const member = await prisma.member.findFirst({
      where: { organizationId: current.organizationId, cpf: parsed.data!.cpf },
      select: { id: true },
    });
    if (member)
      return NextResponse.json({ ok: false, message: "Este CPF já está cadastrado como membro." }, { status: 409 });
    ...
    const code = typeof error === "object" && error && "code" in error ? String(error.code) : "";
    if (code === "P2002")
      return NextResponse.json({ ok: false, message: "Este CPF já possui uma inscrição." }, { status: 409 });
    return databaseErrorResponse(error);
  }
}
