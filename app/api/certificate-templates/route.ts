...
export async function GET() {
  try {
    ...
    const templates = await prisma.documentTemplate.findMany({
      where: {
        organizationId: context.auth!.profile.organizationId,
        type: "CERTIFICATE",
    ...
    console.error("Erro ao listar certificados visuais:", error);
...
export async function POST(request: Request) {
  try {
    const context = await getAdminApiContext();
    if (context.response) return context.response;
    const parsed = await parseJsonRequest(request, certificateTemplateCreateSchema);
    ...
    console.error("Erro ao criar certificado visual:", error);
    return databaseErrorResponse(error);
  }
}
