...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!metadata.success)
      return NextResponse.json({ ok: false, message: "Dados do modelo inválidos." }, { status: 400 });

    let validated;
    try {
      validated = await validateTemplateUpload(file, MAX_TEMPLATE_BYTES);
    } catch {
      return NextResponse.json({ ok: false, message: "Arquivo inválido ou potencialmente inseguro." }, { status: 400 });
    }
    ...
    if (validated.safe.kind === "DOCX") {
      ...
      try {
        analysis = await analyzeDocx(tempPath);
      } catch {
        return NextResponse.json(
          { ok: false, message: "O DOCX contém estrutura ou sintaxe de template não permitida." },
          { status: 400 }
        );
      }
      ...
      let analysis;
      try {
        analysis = await analyzePdf(validated.buffer);
      } catch {
        return NextResponse.json({ ok: false, message: "Não foi possível processar o PDF textual." }, { status: 400 });
      }
    ...
    const template = await prisma.documentTemplate.create({
      data: {
        ...
        extractedText,
        originalFileUrl: storageRef,
        originalFileName: file.name.slice(0, 255),
        originalMimeType: validated.safe.mime,
        ...
        layoutJson:
          validated.safe.kind === "PDF" ? { version: 1, pages: [], coordinateSystem: "pdf-points" } : undefined,
    ...
    return NextResponse.json(
      {
        ...
        requiresOcr: processingStatus === "REQUIRES_OCR",
        needsReview: processingStatus === "NEEDS_REVIEW",
        message:
          processingStatus === "READY"
            ? "Modelo DOCX validado e pronto para uso."
            : processingStatus === "REQUIRES_OCR"
              ? "PDF sem texto suficiente; OCR será necessário."
