...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const document = await prisma.document.findFirst({
      where: { id: params.data.id, organizationId: auth.auth!.profile.organizationId },
    });
    ...
    const reference =
      variant === "signed"
        ? document.signedFile
        : document.fileUrl || document.generatedPdfUrl || document.generatedDocxUrl;
    if (!reference) return NextResponse.json({ ok: false, message: "Arquivo não encontrado." }, { status: 404 });
    return NextResponse.redirect(createPrivateFileUrl(reference), {
      status: 307,
