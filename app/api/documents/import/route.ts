  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { ok: false, message: "Envie um arquivo de até 10 MB e os dados do documento." },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      !parsed.success ||
      [...form.keys()].some((key) => !["file", "metadata"].includes(key))
    ) {
      return NextResponse.json(
        { ok: false, message: "Dados inválidos. Revise arquivo, data, status e vínculos." },
        { status: 400 }
      );
    }
    ...
    if (error instanceof DocumentImportError)
      return NextResponse.json(
        { ok: false, message: error.message, duplicate: error.duplicate },
        { status: error.status }
      );
