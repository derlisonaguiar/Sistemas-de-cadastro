...
export async function POST(request: Request) {
  ...
  try {
    const limited = checkRateLimit(request, "certificate-image", 20, 60_000);
    ...
    if (!(file instanceof File) || file.size <= 0 || file.size > MAX_IMAGE_BYTES)
      return NextResponse.json({ ok: false, message: "Imagem inválida ou acima do limite." }, { status: 400 });
    const buffer = Buffer.from(await file.arrayBuffer());
    let safe;
    try {
      safe = validateImageUpload(buffer, file);
    } catch {
      return NextResponse.json({ ok: false, message: "Imagem inválida." }, { status: 400 });
    }
    if (safe.mime === "image/webp")
      return NextResponse.json({ ok: false, message: "Use PNG ou JPEG no editor de certificados." }, { status: 400 });
    const organizationId = auth.auth!.profile.organizationId;
    const id = randomUUID();
    storageRef = await uploadPrivateObject(
      `organizations/${organizationId}/certificate-assets/${id}/image${safe.extension}`,
      buffer,
      safe.mime
    );
    ...
    return NextResponse.json(
      { ok: true, asset: { ...asset, url: await createSignedStorageUrl(storageRef, 300) } },
      { status: 201 }
    );
  } catch (error) {
    if (storageRef) await removeStorageObject(storageRef).catch(() => undefined);
    console.error("Erro ao enviar imagem de certificado:", error);
