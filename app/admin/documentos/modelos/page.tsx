...
export default function ModelosDocumentosPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [templatesResponse, organizationResponse] = await Promise.all([
  ...
  async function handleDelete(template: Template) {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        setMessage(data.message || "Erro ao excluir modelo.");
    ...
    <div className="max-w-6xl">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Envie e gerencie os modelos usados para gerar documentos.</p>
        </div>

        <div className="flex gap-2">
          <Link
            href="/admin/documentos/modelos/certificados"
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        {loading ? (
          <div className="p-6 text-sm text-gray-600">Carregando modelos...</div>
        ) : templates.length === 0 ? (
          <div className="p-10 text-center">
            <h2 className="font-medium text-gray-900">Nenhum modelo cadastrado</h2>

            <p className="mt-2 text-sm text-gray-500">Envie um arquivo DOCX para criar seu primeiro modelo.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  <th className="px-5 py-3 font-medium text-gray-600">Modelo</th>

                  <th className="px-5 py-3 font-medium text-gray-600">Tipo</th>

                  <th className="px-5 py-3 font-medium text-gray-600">Arquivo original</th>
                ...
                {templates.map((template) => (
                  <tr key={template.id}>
                    <td className="px-5 py-4">
                      ...
                      <p className="mt-1 text-xs text-gray-500">{template.description || "Sem descrição"}</p>
                    </td>

                    <td className="px-5 py-4 text-gray-700">{typeLabel(template.type)}</td>

                    <td className="px-5 py-4 text-gray-700">{template.originalFileName || "Criado manualmente"}</td>
