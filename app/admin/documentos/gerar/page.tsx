...
type MissingField = {
  source: "member" | "representative" | "organization";
  key: string;
  label: string;
};
...
function getMemberValue(member: Member, key: string): unknown {
  if (key === "position") {
...
function getOrganizationValue(organization: Organization, key: string): unknown {
...
function GerarDocumentoContent() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const [templatesResponse, membersResponse, organizationResponse] = await Promise.all([
        ...
        if (!templatesResponse.ok || !templatesData.ok) {
          throw new Error(templatesData.message || "Erro ao carregar modelos.");
        }

        if (!membersResponse.ok || !membersData.ok) {
          throw new Error(membersData.message || "Erro ao carregar membros.");
        }

        if (!organizationResponse.ok || !organizationData.ok) {
          throw new Error(organizationData.message || "Erro ao carregar organização.");
        }
        ...
        setMessage("Erro ao carregar os dados necessários para gerar documentos.");
      } finally {
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const memberExists = members.some((member) => member.id === memberIdFromUrl);
  ...
  useEffect(() => {
    ...
    const templateExists = templates.some((template) => template.id === templateIdFromUrl);
  ...
  const representatives = useMemo(() => {
    return members.filter(
      (member) =>
        ...
        (member.position?.role === "PRESIDENT" || member.position?.role === "VICE_PRESIDENT")
    );
  }, [members]);
  ...
  const selectedRepresentative = representatives.find((member) => member.id === selectedRepresentativeId) || null;

  const templateUsesRepresentative =
    selectedTemplate?.fields.some((field) => field.key.startsWith("representative.")) || false;

  const manualFields =
    selectedTemplate?.fields.filter(
      (field) =>
        (!field.mappedPath || field.mappedPath === "manual") &&
        !field.key.startsWith("member.") &&
        !field.key.startsWith("representative.") &&
        !field.key.startsWith("organization.") &&
        !field.key.startsWith("system.")
    ) || [];
  ...
  const missingFields = useMemo(() => {
    ...
    for (const field of selectedTemplate.fields) {
      ...
      if (field.key.startsWith("member.")) {
        if (!selectedMember) {
          continue;
        }

        const key = field.key.replace("member.", "");

        const value = getMemberValue(selectedMember, key);
      ...
      if (field.key.startsWith("representative.")) {
        if (!selectedRepresentative) {
          continue;
        }

        const key = field.key.replace("representative.", "");

        const value = getMemberValue(selectedRepresentative, key);
      ...
      if (field.key.startsWith("organization.")) {
        if (!organization) {
          continue;
        }

        const key = field.key.replace("organization.", "");

        const value = getOrganizationValue(organization, key);
    ...
    return missing;
  }, [selectedTemplate, selectedMember, selectedRepresentative, organization]);

  const missingMemberFields = missingFields.filter((field) => field.source === "member");

  const missingRepresentativeFields = missingFields.filter((field) => field.source === "representative");

  const missingOrganizationFields = missingFields.filter((field) => field.source === "organization");
  ...
  function handleManualValueChange(key: string, value: string) {
  ...
  async function handleGenerateDocument() {
    if (!selectedTemplate) {
      setMessage("Selecione um modelo.");

      return;
    }

    if (!selectedMember) {
      setMessage("Selecione um membro.");

      return;
    }

    if (templateUsesRepresentative && !selectedRepresentative) {
      setMessage("Selecione o representante da organização.");

      return;
    }

    if (missingFields.length > 0) {
      setMessage("Existem dados obrigatórios faltando. Complete os cadastros antes de gerar o documento.");
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        setMessage(data.message || "Erro ao gerar documento.");

        return;
      }

      setGeneratedDocxUrl(`/admin/documentos/${data.document.id}`);
  ...
  return (
    <div className="max-w-5xl">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Gerar documento</h1>

        <p className="mt-1 text-sm text-gray-600">
          Escolha um modelo, um membro e, quando necessário, o representante da organização.
        </p>
      </div>

      {memberIdFromUrl && selectedMember && (
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-md border border-[var(--admin-accent-border)] bg-[var(--admin-soft)] px-4 py-3">
          <div>
            <p className="text-xs font-medium uppercase text-[var(--admin-ink)]">Documento para</p>

            <p className="mt-1 text-sm font-medium text-[var(--admin-ink)]">{selectedMember.fullName}</p>
          </div>
      ...
      {generatedDocxUrl && (
        <div className="mb-5 rounded-md border border-green-200 bg-green-50 px-4 py-4">
          <p className="text-sm font-medium text-green-800">Documento pronto.</p>
      ...
      <div className="space-y-6">
        <section className="rounded-lg border border-gray-200 bg-white">
          <div className="border-b border-gray-200 px-5 py-4">
            <h2 className="font-semibold text-gray-900">Dados principais</h2>
          </div>
          ...
          <div className="grid gap-5 p-5 md:grid-cols-2">
            ...
            {templateUsesRepresentative && (
              <div className="md:col-span-2">
                <label className="mb-1 block text-sm font-medium text-gray-700">Representante da organização</label>

                {representatives.length === 0 ? (
                  <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    Nenhum Presidente ou Vice-Presidente ativo foi encontrado.
                  </div>
                ) : (
                  <select
                    ...
                    className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800"
                  >
                    {representatives.length > 1 && <option value="">Selecione o representante</option>}
        ...
        {selectedTemplate && missingFields.length > 0 && (
          <section className="rounded-lg border border-amber-300 bg-amber-50">
            <div className="border-b border-amber-200 px-5 py-4">
              <h2 className="font-semibold text-amber-900">Dados incompletos</h2>

              <p className="mt-1 text-sm text-amber-800">
                Alguns dados obrigatórios do modelo não estão preenchidos. Corrija o cadastro correspondente antes de
                gerar o documento.
              </p>
            </div>

            <div className="space-y-4 p-5">
              {missingMemberFields.length > 0 && selectedMember && (
                <div className="rounded-md border border-amber-200 bg-white p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">Dados do membro</p>

                      <p className="mt-1 text-xs text-gray-500">{selectedMember.fullName}</p>

                      <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-amber-900">
                        {missingMemberFields.map((field) => (
              ...
              {missingRepresentativeFields.length > 0 && selectedRepresentative && (
                <div className="rounded-md border border-amber-200 bg-white p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">Dados do representante</p>

                      <p className="mt-1 text-xs text-gray-500">
                        {selectedRepresentative.fullName}

                        {selectedRepresentative.position?.name ? ` — ${selectedRepresentative.position.name}` : ""}
                      </p>

                      <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-amber-900">
                        {missingRepresentativeFields.map((field) => (
                          <li key={field.key}>{field.label}</li>
                        ))}
                      </ul>
                    </div>
              ...
              {missingOrganizationFields.length > 0 && (
                <div className="rounded-md border border-amber-200 bg-white p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">Dados da organização</p>

                      <p className="mt-1 text-xs text-gray-500">{organization?.name || "Organização"}</p>

                      <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-amber-900">
                        {missingOrganizationFields.map((field) => (
                          <li key={field.key}>{field.label}</li>
                        ))}
                      </ul>
                    </div>
          ...
          <section className="rounded-lg border border-gray-200 bg-white">
            <div className="border-b border-gray-200 px-5 py-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h2 className="font-semibold text-gray-900">Campos manuais</h2>

                  <p className="mt-1 text-sm text-gray-500">
                    Apenas campos que não possuem uma origem automática no sistema aparecem aqui.
                  </p>
                </div>

                <span className="rounded-full bg-[var(--admin-soft)] px-3 py-1 text-xs font-medium text-[var(--admin-ink)]">
                  {manualFields.length} campos
                </span>
              </div>
            </div>

            {manualFields.length === 0 ? (
              <div className="p-5 text-sm text-gray-500">Este modelo não possui campos manuais.</div>
            ) : (
              <div className="grid gap-5 p-5 md:grid-cols-2">
                {manualFields.map((field) => (
                  <div key={field.id}>
                    ...
                    <input
                      ...
                      placeholder={fieldLabels[field.key] || field.label}
                      className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800"
                    />
                  </div>
                ))}
        ...
        <div className="flex justify-end pb-8">
          <button
            ...
            disabled={
              !selectedTemplateId ||
              !selectedMemberId ||
              (templateUsesRepresentative && !selectedRepresentativeId) ||
...
export default function GerarDocumentoPage() {
  return (
    <Suspense fallback={<GerarDocumentoLoading />}>
