...
import {
  documentEntityLabels,
  documentStatusLabels,
  documentTypeLabels,
  type DocumentEntityKey,
...
function linkedTo(document: Document) {
  return (
    [
      ...
      document.organizationDocument ? "Organização" : null,
    ]
      .filter(Boolean)
      .join(" · ") || "Sem vínculo específico"
  );
}
...
export default function DocumentosPage() {
          ...
          return entity
            ? [
                [
                  ...
                  {
                    id: entity.id,
                    label: "fullName" in entity ? entity.fullName : "name" in entity ? entity.name : entity.title,
                  },
                ] as const,
              ]
            : [];
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [documentsResponse, templatesResponse, organizationResponse] = await Promise.all([
          fetch("/api/documents"),
  ...
  const filteredDocuments = documents.filter((document) => {
    ...
    if (filters.documentDate && (document.documentDate || document.issueDate)?.slice(0, 10) !== filters.documentDate)
      return false;
    ...
    return (
      ...
      typeLabel(document.type).toLowerCase().includes(term) ||
      statusLabel(document.status).toLowerCase().includes(term) ||
      linkedTo(document).toLowerCase().includes(term)
    );
  });
    ...
    <div className="max-w-6xl">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Gere documentos a partir de modelos e acompanhe o histórico.</p>
        </div>

        <div className="flex flex-wrap gap-2">
          <AdminOnly>
            <Link
              href="/admin/documentos/importar"
      ...
      <AdminOnly>
        <section className="mb-6">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <h2 className="font-semibold text-gray-900">Modelos disponíveis</h2>

              <p className="mt-1 text-sm text-gray-500">Escolha um modelo para gerar um novo documento.</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="rounded-lg border border-dashed border-gray-300 bg-white p-8 text-center">
              <h3 className="font-medium text-gray-900">Nenhum modelo disponível</h3>

              <p className="mt-2 text-sm text-gray-500">Envie um modelo DOCX para começar.</p>
                ...
                <div key={template.id} className="rounded-lg border border-gray-200 bg-white p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-gray-500">{typeLabel(template.type)}</p>
                    </div>

                    <span className="rounded-full bg-green-50 px-2 py-1 text-xs font-medium text-green-700">Ativo</span>
                  </div>

                  <p className="mt-4 min-h-10 text-sm text-gray-600">
                    {template.description || "Modelo pronto para geração."}
                  </p>
      ...
      <section>
        <div className="mb-3">
          ...
          <p className="mt-1 text-sm text-gray-500">Histórico dos documentos gerados e importados.</p>
        </div>
        ...
        <div className="mb-4 grid gap-3 md:grid-cols-3">
          <label className="text-sm">
            ...
            <select
              ...
              onChange={(e) => changeFilter("origin", e.target.value)}
          ...
          {(Object.keys(documentEntityLabels) as DocumentEntityKey[]).map((key) => (
            <label key={key} className="text-sm">
              {documentEntityLabels[key]}
              <select
                value={filters[key]}
          ...
          <label className="text-sm">
            ...
            <input
              ...
              onChange={(e) => changeFilter("documentDate", e.target.value)}
              className="block w-full rounded-md border p-2"
            />
          </label>
        </div>
        ...
        <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
          {loading ? (
            <div className="p-6 text-sm text-gray-600">Carregando documentos...</div>
          ) : filteredDocuments.length === 0 ? (
            <div className="p-10 text-center">
              <h3 className="font-medium text-gray-900">Nenhum documento encontrado</h3>

              <p className="mt-2 text-sm text-gray-500">Os documentos gerados e importados aparecerão aqui.</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="border-b border-gray-200 bg-gray-50">
                  <tr>
                    <th className="px-5 py-3 font-medium text-gray-600">Documento</th>

                    <th className="px-5 py-3 font-medium text-gray-600">Tipo</th>

                    <th className="px-5 py-3 font-medium text-gray-600">Vinculado a</th>
                      ...
                      <td className="px-5 py-4">
                        <p className="font-medium text-gray-900">
                          ...
                          <span className="mt-1 block text-xs text-gray-500">
                            {(document.documentDate || document.issueDate)
                              ?.slice(0, 10)
                              .split("-")
                              .reverse()
                              .join("/") || "Sem data"}
                        ...
                        {document.generatedDocxUrl && <p className="mt-1 text-xs text-green-600">DOCX gerado</p>}
                      </td>

                      <td className="px-5 py-4 text-gray-700">{typeLabel(document.type)}</td>

                      <td className="px-5 py-4 text-gray-700">{linkedTo(document)}</td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          {document.generatedDocxUrl && (
                            <a href={document.generatedDocxUrl} className="font-medium text-gray-700">
                              Baixar DOCX
                            </a>
                          )}
                        </div>
