...
export default function NovoMembroPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [directoratesResponse, positionsResponse] = await Promise.all([
        ...
        setMessage("Erro ao carregar diretorias e cargos.");
      } finally {
        setLoading(false);
      }
    }
    ...
    if (selectedPosition.role === "PRESIDENT") {
      return {
        ...
        text:
          ...
          "Se outro membro já ocupar essa função, o cadastro será bloqueado.",
      };
    }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        setMessage(data.message || "Erro ao cadastrar membro.");
      ...
      if (data.member?.id) {
        ...
        router.push(`/admin/membros/${data.member.id}`);
  ...
  if (loading) {
    return <div className="text-sm text-gray-600">Carregando dados...</div>;
  }

  return (
    <div className="max-w-5xl">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Novo membro</h1>

        <p className="mt-1 text-sm text-gray-600">
          Cadastre os dados pessoais, acadêmicos e organizacionais do membro.
        </p>
      </div>
