...
export default function ContratoPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [contractResponse, organizationResponse] = await Promise.all([
        ...
        if (contractData.ok) {
          ...
          setMessage(contractData.message || "Contrato não encontrado.");
        }
  ...
  async function handleDelete() {
    ...
    const confirmed = window.confirm(`Deseja realmente excluir o contrato "${contract.title}"?`);
  ...
  if (loading) {
    return <div className="text-sm text-gray-600">Carregando contrato...</div>;
  }
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {contract.contractNumber ? `Contrato ${contract.contractNumber}` : "Contrato sem número"}
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-lg border border-gray-200 bg-white">
          <div className="border-b border-gray-200 px-5 py-4">
            <h2 className="font-semibold text-gray-900">Vínculos</h2>
          </div>

          <div className="grid gap-5 p-5 sm:grid-cols-2">
            <div>
              <p className="text-xs font-medium uppercase text-gray-500">Cliente</p>
              <p className="mt-1 text-sm text-gray-900">{contract.client.name}</p>
            </div>

            <div>
              <p className="text-xs font-medium uppercase text-gray-500">Projeto</p>
              <p className="mt-1 text-sm text-gray-900">{contract.project?.name || "Sem projeto"}</p>
            </div>

            <div>
              <p className="text-xs font-medium uppercase text-gray-500">Valor</p>
              <p className="mt-1 text-sm text-gray-900">{formatCurrency(contract.value)}</p>
            </div>

            <div>
              <p className="text-xs font-medium uppercase text-gray-500">Status</p>
              <p className="mt-1 text-sm text-gray-900">{statusLabel(contract.status)}</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-lg border border-gray-200 bg-white">
          <div className="border-b border-gray-200 px-5 py-4">
            <h2 className="font-semibold text-gray-900">Datas</h2>
          </div>
          ...
          <div className="grid gap-5 p-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900">{formatDate(contract.signatureDate)}</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-lg border border-gray-200 bg-white lg:col-span-2">
          ...
          <div className="p-5">
            <p className="whitespace-pre-wrap text-sm text-gray-700">
              {contract.description || "Nenhuma descrição cadastrada."}
            </p>
          </div>
        </section>
