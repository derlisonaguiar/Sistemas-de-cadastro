...
export default function EditarContratoPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [contractResponse, clientsResponse, projectsResponse] = await Promise.all([
        ...
        if (!contractResponse.ok || !contractData.ok) {
          setMessage(contractData.message || "Erro ao carregar contrato.");
          return;
        }
        ...
        setForm({
          ...
          signatureDate: contract.signatureDate ? contract.signatureDate.slice(0, 10) : "",
  ...
  const filteredProjects = form.clientId
    ? projects.filter((project) => !project.client || project.client.id === form.clientId)
    : projects;
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        setMessage(data.message || "Erro ao atualizar contrato.");
        return;
      }
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Editar contrato</h1>

        <p className="mt-1 text-sm text-gray-600">Atualize os dados do contrato.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <section className="rounded-lg border border-gray-200 bg-white">
          <div className="grid gap-4 p-5 md:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Data da assinatura</label>
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Data de término</label>
        ...
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={() => router.push(`/admin/contratos/${id}`)}
