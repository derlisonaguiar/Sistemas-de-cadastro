...
export default function ContratosPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [contractsResponse, organizationResponse] = await Promise.all([
          fetch("/api/contracts"),
    ...
    <div className="max-w-6xl">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Gerencie os contratos de clientes e projetos.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        {loading ? (
          <div className="p-6 text-sm text-gray-600">Carregando contratos...</div>
        ) : filteredContracts.length === 0 ? (
          <div className="p-8 text-center text-sm text-gray-500">Nenhum contrato encontrado.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  <th className="px-5 py-3 font-medium text-gray-600">Contrato</th>

                  <th className="px-5 py-3 font-medium text-gray-600">Cliente</th>

                  <th className="px-5 py-3 font-medium text-gray-600">Projeto</th>

                  <th className="px-5 py-3 font-medium text-gray-600">Valor</th>
                ...
                {filteredContracts.map((contract) => (
                  <tr key={contract.id}>
                    <td className="px-5 py-4">
                      ...
                      <p className="text-xs text-gray-500">{contract.contractNumber || "Sem número"}</p>
                    </td>

                    <td className="px-5 py-4 text-gray-700">{contract.client.name}</td>

                    <td className="px-5 py-4 text-gray-700">{contract.project?.name || "—"}</td>

                    <td className="px-5 py-4 text-gray-700">{formatCurrency(contract.value)}</td>
