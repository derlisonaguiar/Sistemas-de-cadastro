...
type PositionRole = "PRESIDENT" | "VICE_PRESIDENT" | "DIRECTOR" | "MANAGER" | "MEMBER" | "OTHER";
...
async function fetchPositionOptions() {
  ...
  return {
    ...
    directorates: directoratesData.directorates as Array<{ id: string; name: string }>,
  };
}
...
export default function CargosPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setMessage(editingId ? "Cargo atualizado com sucesso." : "Cargo cadastrado com sucesso.");
  ...
  async function handleDelete(position: Position) {
    const confirmed = window.confirm(`Deseja realmente excluir o cargo "${position.name}"?`);
  ...
  return (
    <div className="max-w-5xl">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-600">Cadastre e gerencie os cargos da organização.</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        <form onSubmit={handleSubmit} className="h-fit rounded-lg border border-gray-200 bg-white">
          <div className="border-b border-gray-200 px-5 py-4">
            <h2 className="font-semibold text-gray-900">{editingId ? "Editar cargo" : "Novo cargo"}</h2>
          </div>
          ...
          <div className="space-y-4 p-5">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Tipo do cargo *</label>

              <select
                value={role}
                onChange={(event) => setRole(event.target.value as PositionRole)}
                required
                className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
              >
                <option value="PRESIDENT">Presidência</option>

                <option value="VICE_PRESIDENT">Vice-Presidência</option>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Essa classificação é usada pelo sistema para identificar presidência, diretoria e outras funções.
              </p>
            </div>
            ...
            <div>
              <label htmlFor="position-directorate" className="mb-1 block text-sm font-medium text-gray-700">
                Diretoria do cargo
              </label>
              <select
                id="position-directorate"
                value={directorateId}
              ...
              <p className="mt-1 text-xs text-gray-500">
                Quando vinculado, o cargo só pode ser atribuído a membros desta diretoria.
              </p>
            </div>
        ...
        <section className="rounded-lg border border-gray-200 bg-white">
          <div className="border-b border-gray-200 px-5 py-4">
            <h2 className="font-semibold text-gray-900">Cargos cadastrados</h2>
          </div>

          {loading ? (
            <div className="p-5 text-sm text-gray-600">Carregando...</div>
          ) : positions.length === 0 ? (
            <div className="p-8 text-center text-sm text-gray-500">Nenhum cargo cadastrado.</div>
          ) : (
            <div className="divide-y divide-gray-200">
              {positions.map((position) => (
                <div key={position.id} className="flex items-start justify-between gap-4 px-5 py-4">
                  <div>
                    <p className="font-medium text-gray-900">{position.name}</p>

                    <p className="mt-1 text-sm text-gray-500">{position.description || "Sem descrição"}</p>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      <span className="inline-block rounded-full bg-[var(--admin-soft)] px-2 py-1 text-xs font-medium text-[var(--admin-ink)]">
                        ...
                        {position.directorateId
                          ? ` · ${directorates.find((item) => item.id === position.directorateId)?.name || "Diretoria"}`
                          : ""}
                      </span>

                      <span
                        className={`inline-block rounded-full px-2 py-1 text-xs font-medium ${
                          position.active ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-600"
                        }`}
                      >
