...
const fields = [
  ...
  ["phone", "Telefone", "text"],
  ["course", "Curso", "text"],
  ["registration", "Matrícula", "text"],
  ["nationality", "Nacionalidade", "text"],
  ["maritalStatus", "Estado civil", "text"],
  ["rg", "RG", "text"],
  ["rgIssuer", "Órgão emissor", "text"],
...
type Application = typeof emptyForm & {
  id: string;
...
export default function InscricaoPage() {
  ...
  useEffect(() => {
    ...
    async function loadApplication() {
      try {
        ...
        setForm(
          Object.fromEntries(
            Object.keys(emptyForm).map((key) => [key, data.application[key] || ""])
          ) as typeof emptyForm
        );
      } finally {
  ...
  if (loading)
    return <main className="min-h-screen bg-gray-50 p-8 text-sm text-gray-600">Carregando inscrição...</main>;
          ...
          <div className="mt-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            {application.status === "PENDING" && (
              <p className="text-sm text-gray-600">
                Você receberá acesso após a aprovação da administração. Esta página será atualizada automaticamente.
              </p>
            )}
            {application.status === "APPROVED" && (
              <Link
                href="/login"
                className="inline-flex rounded-md bg-purple-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-purple-800"
              >
                Ir para o login
              </Link>
            )}
            {application.status === "REJECTED" && (
              <p className="text-sm text-gray-600">Não é possível acessar a organização com uma inscrição rejeitada.</p>
            )}
          </div>
        ) : (
          <form onSubmit={submit} className="mt-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <div className="grid gap-4 md:grid-cols-2">
              {fields.map(([key, label, type]) => (
                <label
                  key={key}
                  className={`block text-sm font-medium text-gray-700 ${key === "fullName" || key === "address" ? "md:col-span-2" : ""}`}
                >
                  {label}
                  <input
                    type={type}
                    required={key === "fullName" || key === "cpf"}
                    value={form[key]}
              ...
              <div className="md:col-span-2">
                <p className="mb-1 text-sm font-medium text-gray-700">Foto (opcional)</p>
                <FileUploadField
            ...
            <button
              disabled={saving}
              className="mt-6 rounded-md bg-purple-700 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-60"
            >
