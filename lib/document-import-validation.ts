...
export const importDocumentSchema = z
  ...
  .strict()
  .refine((data) => data.status !== "SIGNED" || data.signed, {
    message: "Marque o arquivo como já assinado.",
    path: ["signed"],
  })
  .refine((data) => !data.signed || ["SIGNED", "ARCHIVED", "CANCELED"].includes(data.status), {
    message: "Arquivo assinado deve ter status Assinado, Arquivado ou Cancelado.",
    path: ["status"],
  })
  .refine((data) => !data.duplicateOfId || data.duplicateReason.length >= 10, {
    message: "Informe uma justificativa de pelo menos 10 caracteres.",
  ...
  if (result.safe.kind === "PDF") {
    ...
    for (const entry of Object.values(zip.files)) {
      ...
      if (/\.(xml|rels)$/i.test(entry.name)) {
        ...
        if (
          /\x00|<!DOCTYPE|<!ENTITY|macroEnabled|oleObject|altChunk|TargetMode\s*=\s*["']External|\bDDE(?:AUTO)?\b/i.test(
            xml
          )
        )
