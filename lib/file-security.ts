...
export function validateDocx(buffer: Buffer, file: File): SafeUpload {
  ...
  for (const entry of entries) {
    ...
    if (compressedSize > 0 && size / compressedSize > MAX_COMPRESSION_RATIO)
      throw new Error("DOCX_SUSPICIOUS_COMPRESSION");
    total += size;
    if (total > MAX_TOTAL_UNCOMPRESSED) throw new Error("DOCX_UNCOMPRESSED_TOO_LARGE");
  }
  if (buffer.length > 0 && total / buffer.length > MAX_COMPRESSION_RATIO)
    throw new Error("DOCX_SUSPICIOUS_COMPRESSION");
  return {
    kind: "DOCX",
...
export function validatePdf(buffer: Buffer, file: File): SafeUpload {
  ...
  if (!buffer.subarray(Math.max(0, buffer.length - 2048)).includes(Buffer.from("%%EOF")))
    throw new Error("INVALID_PDF_EOF");
...
export function validateTemplateUpload(file: File, maxBytes: number) {
  ...
  return file.arrayBuffer().then((value) => {
    ...
    return {
      buffer,
      safe: file.name.toLowerCase().endsWith(".pdf") ? validatePdf(buffer, file) : validateDocx(buffer, file),
    };
  });
}
...
export function validateImageUpload(buffer: Buffer, file: File) {
  ...
  if (buffer.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))) {
    if (buffer.length < 33 || buffer.subarray(12, 16).toString("ascii") !== "IHDR")
      throw new Error("INVALID_PNG_STRUCTURE");
