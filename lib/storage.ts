...
function safePath(objectPath: string) {
  ...
  if (
    ...
    !/^organizations\/[A-Za-z0-9_-]+\//.test(normalized)
  )
...
export function storageReference(_bucket: string, objectPath: string) {
  return `${PREFIX}${safePath(objectPath).normalized}`;
}
export function parseStorageReference(reference: string | null | undefined) {
  if (!reference?.startsWith(PREFIX)) return null;
  try {
    return { path: safePath(reference.slice(PREFIX.length)).normalized };
  } catch {
    return null;
  }
}
export async function uploadPrivateObject(objectPath: string, data: Buffer, _contentType: string) {
  const target = safePath(objectPath);
  ...
  try {
    ...
    throw new Error("Falha ao armazenar arquivo privado.");
  }
  return `${PREFIX}${target.normalized}`;
}
export async function uploadPublicObject(objectPath: string, data: Buffer, contentType: string) {
  return createPrivateFileUrl(await uploadPrivateObject(objectPath, data, contentType));
}
export async function downloadStorageObject(reference: string) {
  const parsed = parseStorageReference(reference);
  if (!parsed) return null;
  try {
    return await readFile(safePath(parsed.path).resolved);
  } catch {
    throw new Error("Arquivo privado não encontrado.");
  }
}
export function createPrivateFileUrl(reference: string) {
  if (!parseStorageReference(reference)) throw new Error("Referência de arquivo inválida.");
  return `/api/files?ref=${encodeURIComponent(reference)}`;
}
/** @deprecated Local compatibility name; returns an authenticated local route, never a signed URL. */
export async function createSignedStorageUrl(reference: string, _expiresIn = 300) {
  return createPrivateFileUrl(reference);
}
export async function removeStorageObject(reference: string) {
  const parsed = parseStorageReference(reference);
  if (parsed) await rm(safePath(parsed.path).resolved, { force: true });
}
