...
export async function renderCertificatePdf(
  ...
  images: Map<string, CertificateImage> = new Map()
) {
  ...
  const sx = pageWidth / layout.page.width;
  const sy = pageHeight / layout.page.height;
  ...
  async function fontFor(family: string, bold: boolean, italic: boolean) {
    ...
    const embedded = await document.embedFont(variants[bold && italic ? 3 : bold ? 1 : italic ? 2 : 0]);
  ...
  async function drawImage(
    ...
    const source = images.get(assetId);
    if (!source) return;
    const embedded =
      source.mimeType === "image/png" ? await document.embedPng(source.bytes) : await document.embedJpg(source.bytes);
  ...
  if (layout.background.type === "COLOR")
    page.drawRectangle({ x: 0, y: 0, width: pageWidth, height: pageHeight, color: pdfColor(layout.background.color) });
  ...
  for (const element of layout.elements) {
    ...
    const y = pageHeight - element.y * sy - height;
    const rotation = degrees(-element.rotation);
    if (element.type === "RECTANGLE")
      page.drawRectangle({
        ...
        width,
        height,
        color: pdfColor(element.fill),
        borderColor: pdfColor(element.borderColor),
        borderWidth: element.borderWidth * sx,
        rotate: rotation,
    ...
    if (element.type === "LINE")
      page.drawLine({
        ...
        thickness: element.strokeWidth * sx,
    ...
    if (element.type === "TEXT") {
      ...
      const size = element.fontSize * sy;
      const lineHeight = size * 1.18;
      const lines = wrapText(replaceTemplateText(element.text, values), font, size, width).slice(
        0,
      ...
      lines.forEach((line, index) => {
        ...
        if (element.underline)
          page.drawLine({
            start: { x: x + offset, y: lineY - 1 },
            end: { x: x + offset + lineWidth, y: lineY - 1 },
            thickness: Math.max(0.5, size / 20),
            color: pdfColor(element.color),
          });
      });
    }
