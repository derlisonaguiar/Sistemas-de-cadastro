...
const optionalMoney = z
  .union([z.coerce.number().finite().nonnegative().max(9999999999.99), z.literal(""), z.null()])
  .optional()
  .transform((value) => (value === "" || value == null ? null : value));
...
export const organizationSchema = z
  .object({
    ...
    stateCode: z.union([z.string().trim().length(2), z.literal(""), z.null()]).optional(),
...
export const memberSchema = z
  ...
  .strict()
  .refine((data) => !data.entryDate || !data.exitDate || data.exitDate >= data.entryDate, {
    message: "A data de saída não pode ser anterior à entrada.",
    path: ["exitDate"],
  });

export const memberApplicationSchema = z
  .object({
    fullName: requiredText(200),
    email: optionalEmail,
    cpf: requiredCpf,
...
export const initialSetupSchema = initialSetupOrganizationSchema
  .extend({
  ...
  .refine((data) => data.adminEmail === data.adminEmailConfirmation, {
...
export const memberApplicationReviewSchema = memberApplicationSchema
  .extend({
    directorateId: optionalIdSchema,
    positionId: optionalIdSchema,
  })
...
export const memberApplicationDecisionSchema = z
  .object({
...
export const organizationEntryCodeSchema = z
  .object({
    entryCode: z.string().trim().toUpperCase().length(6, "O código de ingresso deve ter 6 caracteres."),
  })
  .strict();

export const directorateSchema = z
  .object({
...
export const templateFieldSchema = z
  .object({
    fieldId: idSchema,
    label: requiredText(160).optional(),
    type: z
      .enum(["TEXT", "NUMBER", "DATE", "CPF", "CNPJ", "EMAIL", "PHONE", "ADDRESS", "CURRENCY", "BOOLEAN"])
      .optional(),
    required: z.boolean().optional(),
    mappedPath: z.string().trim().max(160).optional(),
  })
  .strict();

export const documentGenerationSchema = z
  .object({
    ...
    representativeId: optionalIdSchema,
    manualValues: z
...
export const createLocalOrganizationSchema = z
  .object({ mode: z.literal("create"), organizationName: requiredText(160) })
  .strict();
