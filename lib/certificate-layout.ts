...
const imageElement = z
  .object({ ...common, type: z.literal("IMAGE"), assetId: id, lockAspectRatio: z.boolean().default(true) })
  .strict();
const lineElement = z
  .object({ ...common, type: z.literal("LINE"), color, strokeWidth: z.number().finite().min(1).max(40) })
  .strict();
const rectangleElement = z
  .object({
    ...common,
...
export const certificateLayoutSchema = z
  ...
  .superRefine((layout, context) => {
    ...
    if (new Set(ids).size !== ids.length)
      context.addIssue({ code: "custom", path: ["elements"], message: "IDs de elementos devem ser únicos." });
  });
...
export function certificateAssetIds(layout: CertificateLayout) {
  const ids = layout.elements
    .filter((item): item is Extract<CertificateElement, { type: "IMAGE" }> => item.type === "IMAGE")
