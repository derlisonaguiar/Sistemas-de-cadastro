...
export class DocumentImportError extends Error {
  constructor(
    ...
    message: string,
    public duplicate?: { id: string; title: string }
  ) {
    super(message);
  }
}
...
export async function validateDocumentLinks(db: Prisma.TransactionClient, organizationId: string, links: Links) {
  const [member, client, project, contract] = await Promise.all([
    links.memberId
      ? db.member.findFirst({ where: { id: links.memberId, organizationId }, select: { id: true } })
      : null,
    links.clientId
      ? db.client.findFirst({ where: { id: links.clientId, organizationId }, select: { id: true } })
      : null,
    links.projectId
      ? db.project.findFirst({ where: { id: links.projectId, organizationId }, select: { id: true, clientId: true } })
      : null,
    links.contractId
      ? db.contract.findFirst({
          where: { id: links.contractId, organizationId },
          select: { id: true, clientId: true, projectId: true },
        })
      : null,
  ]);
  if (
    (links.memberId && !member) ||
    (links.clientId && !client) ||
    (links.projectId && !project) ||
    (links.contractId && !contract)
  ) {
    throw new DocumentImportError(400, "Vínculo inválido para esta organização.");
  }
  if (
    (client && project?.clientId && project.clientId !== client.id) ||
...
export async function importDocument(
  ...
  importedById: string,
  input: z.infer<typeof importDocumentSchema>,
  file: File
) {
  let validated: Awaited<ReturnType<typeof validateImportedFile>>;
  try {
    validated = await validateImportedFile(file);
  } catch {
    throw new DocumentImportError(400, "Envie um PDF ou DOCX seguro, válido e não criptografado, de até 10 MB.");
  }
      ...
      async (tx) => {
        ...
        if (!duplicate) {
          ...
          const existingFiles = await tx.document.findMany({
            where: {
              organizationId,
              OR: [{ origin: "GENERATED" }, { origin: "IMPORTED", fileUrl: { not: null }, signedFile: { not: null } }],
            },
            select: {
              id: true,
              title: true,
          ...
          for (const existing of existingFiles) {
            const references = [
              ...new Set(
                [existing.fileUrl, existing.signedFile, existing.generatedDocxUrl, existing.generatedPdfUrl].filter(
        ...
        if (duplicate && (input.duplicateOfId !== duplicate.id || input.duplicateReason.length < 10)) {
          throw new DocumentImportError(
            409,
            "Este arquivo já existe na organização. Abra o documento existente ou justifique uma nova importação.",
            duplicate
          );
        }
        if (!duplicate && input.duplicateOfId)
          throw new DocumentImportError(
            409,
            "A confirmação de duplicidade não corresponde a este arquivo. Envie novamente."
          );
        const id = randomUUID();
        uploaded = await uploadPrivateObject(
          `organizations/${organizationId}/documents/${id}/imported${safe.extension}`,
          buffer,
          safe.mime
        );
        const links = {
        ...
        return tx.document.create({
          data: {
            ...
            fileUrl: input.signed ? null : uploaded,
            signedFile: input.signed ? uploaded : null,
            // signedAt records receipt of the signed file, not an invented signature date.
            signedAt: input.signed ? now : null,
            importedAt: now,
            importedById,
            importedFileHash: hash,
            importedFileName: file.name.replace(/[\x00-\x1f\x7f]/g, "").slice(0, 255),
            importedMimeType: safe.mime,
            importedFileSize: buffer.length,
            importLinks: { ...links, organizationDocument: input.organizationDocument },
            duplicateOfId: duplicate?.id || null,
            duplicateReason: duplicate ? input.duplicateReason : null,
          },
          select: { id: true },
        });
      },
