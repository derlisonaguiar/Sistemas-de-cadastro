...
function setup({ denied = false, found = true, valid = true, failure = false } = {}) {
  ...
  const route = load("app/api/members/[id]/route.ts", {
    ...
    "@/lib/auth": {
      getAdminApiContext: async () =>
        denied
          ...
          : { auth: { user: { id: "admin", email: "admin@example.com" }, profile: { organizationId: "org" } } },
    },
    "@/lib/prisma": {
      prisma: {
        member: {
          findFirst: async (args) => {
            lookups.push(args);
            return found ? { id: "member" } : null;
          },
        },
      },
    },
    "@/lib/admin-password": {
      verifyAdminPassword: async (...args) => {
        passwords.push(args);
        return valid;
      },
    },
    "@/lib/member-update": {
      updateMember: async (args) => {
        writes.push(args);
        if (failure) throw { code: "P2004" };
        return { id: "member", status: args.data.status, directorate: null, position: null };
      },
    },
  });
  ...
  return {
    ...
    passwords,
    lookups,
    call: (body = { status: "POS_JR", password: "test-only" }) =>
      route.PATCH(
...
test("non ADMIN, foreign member and wrong password never write", async () => {
  for (const [options, status] of [
    ...
    if (status === 401)
      assert.deepEqual(await r.json(), { ok: false, message: "Não foi possível confirmar a alteração." });
  }
});
test("reject invalid status, missing password and injected organization", async () => {
  for (const body of [
    { status: "INVALID", password: "x" },
    { status: "ACTIVE" },
    { status: "ACTIVE", password: "x", organizationId: "foreign" },
  ]) {
    const h = setup();
...
test("rate limit blocks sixth password attempt", async () => {
  const h = setup({ valid: false });
  for (let i = 0; i < 5; i++) assert.equal((await h.call()).status, 401);
  assert.equal((await h.call()).status, 429);
  assert.equal(h.passwords.length, 5);
});
...
test("existing proxy rejects cross-origin PATCH", async () => {
  ...
  const response = await proxy(
    new NextRequest("https://example.com/api/members/member", {
      method: "PATCH",
      headers: { host: "example.com", origin: "https://foreign.example" },
    })
  );
