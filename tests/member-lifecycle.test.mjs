  ...
  try {
    ...
    for (const id of [org, otherOrg])
      await db.query('INSERT INTO "Organization" (id, name, "updatedAt") VALUES ($1, $2, now())', [
        id,
        "Teste transacional Pós-Jr",
      ]);
    for (const [id, organizationId] of [
      [dir, org],
      [otherDir, otherOrg],
    ]) {
      await db.query('INSERT INTO "Directorate" (id, "organizationId", name, "updatedAt") VALUES ($1, $2, $3, now())', [
        id,
        organizationId,
        "Diretoria Teste",
      ]);
    ...
    const insertPosition =
      'INSERT INTO "Position" (id, "organizationId", "directorateId", name, role, "updatedAt") VALUES ($1, $2, $3, $4, $5, now())';
    const insertMember =
      'INSERT INTO "Member" (id, "organizationId", "directorateId", "positionId", "fullName", "updatedAt") VALUES ($1, $2, $3, $4, $5, now())';
    ...
    for (const role of ["DIRECTOR", "PRESIDENT", "VICE_PRESIDENT"]) {
      ...
      await db.query(
        'INSERT INTO "Document" (id, "organizationId", "memberId", title, type, "updatedAt") VALUES ($1, $2, $3, $4, $5, now())',
        [document, org, first, "Histórico preservado", "DECLARATION"]
      );
      ...
      const departed = (await db.query('UPDATE "Member" SET status = $2 WHERE id = $1 RETURNING *', [first, "POS_JR"]))
        .rows[0];
      ...
      const unchanged = (
        await db.query(
          'UPDATE "Member" SET "fullName" = $2, "positionId" = $3, "directorateId" = $4 WHERE id = $1 RETURNING *',
          [first, "Pós-Jr consultável", position, dir]
        )
      ).rows[0];
      ...
      assert.equal(
        (await db.query('SELECT id FROM "Document" WHERE id = $1 AND "memberId" = $2', [document, first])).rowCount,
        1
      );
      await db.query('UPDATE "Member" SET status = $2 WHERE id = $1', [second, "POS_JR"]);
      await db.query('UPDATE "Member" SET status = $2, "positionId" = $3, "directorateId" = $4 WHERE id = $1', [
        first,
        "ACTIVE",
        position,
        dir,
      ...
      const again = (
        await db.query('UPDATE "Member" SET status = $2 WHERE id = $1 RETURNING "membershipHistory"', [first, "POS_JR"])
      ).rows[0];
      assert.equal(again.membershipHistory.length, 2);
      await db.query('DELETE FROM "Position" WHERE id = $1', [position]);
      assert.equal(
        (await db.query('SELECT "membershipHistory" FROM "Member" WHERE id = $1', [first])).rows[0].membershipHistory[0]
          .positionName,
        role
      );
    }
    assert.equal(
      (await db.query('SELECT id FROM "Member" WHERE "organizationId" = $1 AND status = $2', [org, "POS_JR"])).rowCount,
      6
    );
  } finally {
