...
test("PostgreSQL row lock serializes concurrent ADMIN changes and preserves last active ADMIN", async () => {
  ...
  const organizationId = randomUUID(),
    ...
    second = randomUUID();
  try {
    await pool.query('INSERT INTO "Organization" (id,name,"updatedAt") VALUES ($1,$2,now())', [
      organizationId,
      "Teste de concorrência de usuários",
    ]);
    for (const id of [first, second])
      await pool.query(
        'INSERT INTO "UserProfile" (id,"organizationId",role,"updatedAt") VALUES ($1,$2,\'ADMIN\',now())',
        [id, organizationId]
      );
    const prisma = {
          ...
          const result = await action({
            ...
            userProfile: {
              findUnique: async ({ where }) =>
                (await client.query('SELECT * FROM "UserProfile" WHERE id=$1', [where.id])).rows[0],
              findFirst: async ({ where }) =>
                (
                  await client.query('SELECT * FROM "UserProfile" WHERE id=$1 AND "organizationId"=$2', [
                    where.id,
              ...
              count: async ({ where }) =>
                Number(
                  (
                    await client.query(
                      'SELECT count(*) FROM "UserProfile" WHERE "organizationId"=$1 AND role=$2 AND active=$3',
                      [where.organizationId, where.role, where.active]
                    )
                  ).rows[0].count
                ),
              update: async ({ where, data }) =>
                (
                  await client.query(
                    'UPDATE "UserProfile" SET role=COALESCE($2::"UserRole",role),active=COALESCE($3,active) WHERE id=$1 RETURNING *',
                    [where.id, data.role ?? null, data.active ?? null]
                  )
                ).rows[0],
            },
          });
